...
# except for that code marked "FOR TESTING ON LAPTOP" which is published at
# https://github.com/benlawraus/anvil_extras
...
def _is_str(key):
    if type(key) is not str:
        msg = f"Keys must be strings when serializing to browser Storage. Found {type(key).__name__}"
        raise TypeError(msg)
    return True
...
class StorageWrapper:
    ...
    def is_available(self):
        ...
        if not self._store.supports(self._driver):
            # we can't rely on this method - it's just for browser support
            return False
        try:
            self._store.length()
...
def _fail_db(*args):
    ...
    # this is asynchronous so use the Promise api
